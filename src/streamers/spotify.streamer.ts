...
export class SpotifyStreamer implements IStreamer {
    ...
    constructor(
        storer: IStorer,
        args: SpotifyStreamerArgs & IStreamerCacheOpts
    ) {
        this.storer = storer
    ...
    public async getAccessToken(
        ...
        if (
            !forceRefresh &&
            existingAccessToken &&
            existingAccessToken.created_at + existingAccessToken.expires_in >
                Date.now()
        ) {
    ...
    public async fetchCurrentlyPlaying(): Promise<Song | null> {
        ...
        const jsonData =
            (await response.json()) as SpotifyCurrrentlyPlayingResponse

        if (
            !jsonData.is_playing ||
            jsonData.currently_playing_type !== 'track'
        ) {
            return this.fetchLastPlayed(accessToken)
        }
    ...
    private async fetchLastPlayed(
        ...
        const lastPlayed = {
            ...
            artiste: lastPlayedTrack.artists
